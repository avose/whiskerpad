...
# ------------ Char position from pixel ------------

def _get_clicked_line(rich_text: RichText, click_y: int, text_area_y: int, line_height: int, wrapped_lines: List) -> Tuple[int, dict]:
    """Determine which line was clicked and return line info."""
    click_y_in_text = click_y - text_area_y
    if click_y_in_text < 0:
        return 0, wrapped_lines[0] if wrapped_lines else None

    line_idx = max(0, int(click_y_in_text // line_height))
    if line_idx >= len(wrapped_lines):
        return len(wrapped_lines) - 1, wrapped_lines[-1] if wrapped_lines else None

    return line_idx, wrapped_lines[line_idx]

def _find_char_position_in_line(line_info: dict, click_x_in_line: int, dc: wx.DC, font_normal: wx.Font, font_bold: wx.Font) -> int:
    """Find the character position within a clicked line."""
    ...
    for segment in line_info['segments']:
        ...
        if x_pos + segment_width >= click_x_in_line:
            return char_pos + _find_char_in_segment(segment, click_x_in_line - x_pos, dc, font_normal, font_bold)
        x_pos += segment_width
        char_pos += len(segment['text'])

    return line_info['end_char']

def char_pos_from_pixel(rich_text: RichText, click_x: int, click_y: int, text_area_x: int, text_area_y: int, available_width: int, dc: wx.DC, font_normal: wx.Font, font_bold: wx.Font, line_height: int) -> int:
    ...
    line_idx, line_info = _get_clicked_line(rich_text, click_y, text_area_y, line_height, wrapped_lines)
    if not line_info:
        return rich_text.char_count()

    click_x_in_line = click_x - text_area_x
    return _find_char_position_in_line(line_info, click_x_in_line, dc, font_normal, font_bold)

# ------------ Pixel position for char ------------
