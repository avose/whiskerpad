...
def add_sibling_after(nb_dir: str, cur_id: str) -> Optional[str]:
    """
    ...
    if parent_id:
        ...
        idx = next(
            (i for i, it in enumerate(items)
             if isinstance(it, dict) and it.get("type") == "child" and it.get("id") == cur_id),
            -1
        )
        insert_index = (idx + 1) if idx >= 0 else len(items)
        return create_node(nb_dir, parent_id=parent_id, title="", insert_index=insert_index)
    ...
    new_id = create_node(nb_dir, parent_id=None, title="")  # appends by default
...
def indent_under_prev_sibling(nb_dir: str, cur_id: str) -> bool:
    """
    ...
    if parent_id:
        ...
        prev = load_entry(nb_dir, prev_id)
        if prev.get("collapsed", False):
            prev["collapsed"] = False
            save_entry(nb_dir, prev)
...
def outdent_to_parent_sibling(nb_dir: str, cur_id: str) -> bool:
    """
    ...
    idx = next(
        (i for i, it in enumerate(pitems)
         if isinstance(it, dict) and it.get("type") == "child" and it.get("id") == cur_id),
        -1
    )
    if idx < 0:
    ...
    grand = load_entry(nb_dir, grand_id)
    gitems = list(grand.get("items", []))
    pidx = next(
        (i for i, it in enumerate(gitems)
         if isinstance(it, dict) and it.get("type") == "child" and it.get("id") == parent_id),
        -1
    )
    insert_index = pidx + 1 if pidx >= 0 else len(gitems)
    gitems.insert(insert_index, {"type": "child", "id": cur_id})
    grand["items"] = gitems
    save_entry(nb_dir, grand)
...
def set_collapsed(nb_dir: str, entry_id: str, collapsed: bool) -> bool:
    """Set the 'collapsed' flag on an entry. Returns True if a change was made."""
    e = load_entry(nb_dir, entry_id)
...
def toggle_collapsed(nb_dir: str, entry_id: str) -> bool:
